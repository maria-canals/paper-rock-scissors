...
logPoints = (winner) => {
    if (USERSPOINTS === 3 && COMPUTERSPOINTS < 3 || COMPUTERSPOINTS === 3 && USERSPOINTS < 3) {
        return;
...
allButtons.forEach(button => button.addEventListener('click', function () {
    ...
    setTimeout(() => { modal.classList.replace('enable-modal', 'disable-modal'), toggleBackdrop(); }, 9000);

    // falta enseñar el resultado final
